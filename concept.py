...
MIN_SIZE_FOR_MOVEMENT = 10000  # This size will not detect small motions like eye blinks
...
while not motion_detected and not Stooper:
    # Initialize frame variables
    start_time = time.time() # Start time for countdown
    motion_start_time = None # To track the time when motion is detected
    ...
    while True:
        ...
        if initial_phase:
            ...
            if time_remaining <= 0:
                ...
                if cv2.waitKey(1) & 0xFF == ord(' '):  # Spacebar stops the program
                    Stooper = True
                    break
        ...
        if reference_frame is not None:
            ...
            if motion_detected:
                ...
                cv2.putText(original_frame, "Motion detected!", (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                cv2.imshow("frame", original_frame)
        ...
        if time_remaining < 0 and not motion_detected:
            ...
            cv2.imshow("frame", original_frame)
            cv2.waitKey(1000)  # Show message for 2 seconds
            break

        # Display frame
        cv2.imshow("frame", original_frame)

        # Exit on pressing 'space'
